public class PattersonLock {
    ...
    private void assignLeaves(TreeNode node) {
        ...
        if (node.left != null)
            assignLeaves(node.left);
        if (node.right != null)
            assignLeaves(node.right);
    }
    ...
    private boolean lockRec(TreeNode node, int threadId) {
        if (!containsThread(node, threadId))
            return false;
        ...
        boolean inLeft = containsThread(node.left, threadId);
        TreeNode next = inLeft ? node.left : node.right;

        if (lockRec(next, threadId)) {
            int side = inLeft ? 0 : 1;
            node.lock.lock(side);
            return true;
        }
    ...
    private boolean unlockRec(TreeNode node, int threadId) {
        ...
        int side = inLeft ? 0 : 1;
        node.lock.unlock(side);

        unlockRec(next, threadId);
        return false;
    }
